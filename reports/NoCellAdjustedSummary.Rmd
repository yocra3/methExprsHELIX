...
allGenesv <- allGenes$sig
names(allGenesv) <- allGenes$TC



allData <- new("topGOdata", 
...
allClass <- runTest(allData, algorithm = "classic", statistic = "fisher")
allElim <- runTest(allData, algorithm = "elim", statistic = "fisher")
allWeigth <- runTest(allData, algorithm = "weight", statistic = "fisher")
allMixed <- runTest(allData, algorithm = "weight01", statistic = "fisher")


allRes <- GenTable(allData, classic = allClass, Elim = allElim, 
                   weight = allWeigth, w0 = allMixed,
                   orderBy = "weight", ranksOf = "classic",
                   topNodes = 2074)
allRes.sel <- allRes %>%
  as_tibble() %>%
  filter(classic < 0.01 & Elim < 0.01 & weight < 0.01 & w0 < 0.01)


showSigOfNodes(allData, score(allClass), firstSigNodes = 15, useInfo = 'all')
showSigOfNodes(allData, score(allElim), firstSigNodes = 5, useInfo = 'all')
showSigOfNodes(allData, score(allWeigth), firstSigNodes = 5, useInfo = 'all')
showSigOfNodes(allData, score(allMixed), firstSigNodes = 5, useInfo = 'all')
```
