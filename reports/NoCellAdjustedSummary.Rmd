---
title: "HELIX meth-expr No Cell adjusted results"
author: 
- Carlos Ruiz 
...
## Create summary tibble
methSum <- methyAnnot %>% 
  group_by(Type, Direction, median_cat) %>%
  summarize(n = n()) %>%
  spread(median_cat, n) %>%
  mutate(Type0 = ifelse(Type == "Non-significant", "Non-significant", "Significant"), 
         tot = sum(high, medium, low))

  
  
methStates <- c("high", "medium", "low")
...
scale_y_continuous(trans = "log2", 
                     breaks = scales::trans_breaks("log2", function(x) round(2^x, 2))) +
  geom_hline(yintercept = 1) +
  scale_x_discrete(name = "Methylation Levels", limits  = methStates) +
  theme_bw() 
```

CpGs associated with gene expression are enriched in CpGs with medium methylation. Low methylation CpGs are depleted in CpGs regulating gene expression, but for CpGs regulating positively. 
```{r}
monoM <- methSum %>%
  filter(Type %in% c("Mono", "Non-significant")) %>%
  group_by(Type) %>%
...
multiM <- methSum %>%
  ...
  as_tibble() %>%
  mutate(par = c("OR", "p.val", "ORlow", "ORhigh")) %>%
  gather("Region", "Value", 1:4) %>%
  mutate(Type = "Multi")

rbind(sigM, monoM, multiM) %>%
  spread(par, Value) %>%
  mutate(Type = factor(Type, levels = c("Significant", "Mono", "Multi"))) %>%
  ggplot(aes(x = Region, y = OR, fill = Type)) + 
  geom_bar(stat = "identity", position=position_dodge()) + 
...
scale_y_continuous(trans = "log2", 
                     breaks = scales::trans_breaks("log2", function(x) round(2^x, 2))) +
  geom_hline(yintercept = 1) +
  scale_x_discrete(name = "CpG Island", limits  = methStates) +
  theme_bw() 
```
...
# showSigOfNodes(allData, score(allClass), firstSigNodes = 15, useInfo = 'all')
```
...
```{r}
invCpGs <- filter(CpGsSum, Direction == "Inverse")$CpG

negGenes <- df %>%
  group_by(TC) %>%
  summarize(sig = factor(
    ifelse(any(CpG %in% invCpGs & sigPair), 1, 0)))
invGOs <- computeGOs(negGenes)

posCpGs <- filter(CpGsSum, Direction == "Positive")$CpG

posGenes <- df %>%
  group_by(TC) %>%
  summarize(sig = factor(
    ifelse(any(CpG %in% posCpGs & sigPair), 1, 0)))
posGOs <- computeGOs(posGenes)


bothCpGs <- filter(CpGsSum, Direction == "Both")$CpG

bothGenes <- df %>%
  group_by(TC) %>%
  summarize(sig = factor(
    ifelse(any(CpG %in% bothCpGs & sigPair), 1, 0)))
bothGOs <- computeGOs(bothGenes)
...
```{r}
monoCpGs <- filter(CpGsSum, Type == "Mono")$CpG

monoGenes <- df %>%
  group_by(TC) %>%
  summarize(sig = factor(
    ifelse(any(CpG %in% monoCpGs & sigPair), 1, 0)))
monoGOs <- computeGOs(monoGenes)

multiCpGs <- filter(CpGsSum, Type == "Multi")$CpG

multiGenes <- df %>%
  group_by(TC) %>%
  summarize(sig = factor(
    ifelse(any(CpG %in% multiCpGs & sigPair), 1, 0)))
multiGOs <- computeGOs(multiGenes)
```
...
ageGenes <- df %>%
  group_by(TC) %>%
  summarize(sig = factor(
    ifelse(any(CpG %in% agedf$ILMNID & sigPair), 1, 0)))
ageGOs <- computeGOs(ageGenes)


decCpGs <- filter(agedf, Dir == "Decreasing")$ILMNID
decGenes <- df %>%
  group_by(TC) %>%
  summarize(sig = factor(
    ifelse(any(CpG %in% decCpGs & sigPair), 1, 0)))
decGOs <- computeGOs(decGenes)

incCpGs <- filter(agedf, Dir == "Increasing")$ILMNID
incGenes <- df %>%
  group_by(TC) %>%
  summarize(sig = factor(
    ifelse(any(CpG %in% incCpGs & sigPair), 1, 0)))
incGOs <- computeGOs(incGenes)
