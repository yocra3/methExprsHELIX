...
rbind(sig, neg, pos, both) %>%
  ...
  mutate(p.val.thres = ifelse(p.val > 0.05, "P > 0.05", 
         ...
         Type = factor(Type, levels = c("Significant", "Negative", "Positive", "Both"))) %>%
  ...
  scale_color_manual(name = "Significance", values = c("black", "grey50", "white")) +
  theme_bw() 
```
...
rbind(sigC, negC, posC, bothC) %>%
  ...
  scale_color_manual(name = "Significance", values = c("black", "grey50", "white")) +     facet_grid(~ Group, scales = "free", space = "free_x") +
...
upset(fromList(list(Negative = negGenes, Positive = posGenes,
